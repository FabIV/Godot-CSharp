...
func _init(i :int, j :int) -> void:
	_camera_projections = []
	for k in range(i):
		var temp_arr: Array[CameraProjectionGD] = []
		for l in range(j):
...
func set_next_camera(cam :SubCameraSystemGD) -> void:
	...
	if _camera_projections[i[0]][i[1]] == null:
		_camera_projections[i[0]][i[1]] = CameraProjectionGD.new(i)
	_camera_projections[i[0]][i[1]].set_camera(cam)
	_next_camera_pos += 1
	
func set_next_view_projection(vp :ViewProjectionGD) -> void:
	var i : Array[int] = get_array_position(_camera_projections, _next_viewport_pos)
	if _camera_projections[i[0]][i[1]] == null:
		_camera_projections[i[0]][i[1]] = CameraProjectionGD.new(i)
	var cp : CameraProjectionGD = _camera_projections[i[0]][i[1]]
	cp.set_projection(vp)
	_next_viewport_pos += 1
	
func set_positions(scale :int, pixel_y :float, cro :CameraRotationOrganizerGD) -> void:
	for i in range(len(_camera_projections)):
		for j in range(len(_camera_projections[i])):
			var cp : CameraProjectionGD = _camera_projections[i][j]
...
func set_relative_position(x1 :int, x2 :int, y1 :int, y2 :int, cro :CameraRotationOrganizerGD) -> void:
	...
	var cp11 :CameraProjectionGD = _camera_projections[1][1]
